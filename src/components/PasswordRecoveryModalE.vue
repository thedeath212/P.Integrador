      ...
      <div class="bg-white p-6 rounded-lg shadow-lg max-w-md w-full">
        ...
        <form @submit.prevent="handleRecovery">
          ...
          <div class="mb-4">
            ...
            <input type="text" id="dni" v-model="ComDni" @input="validateDNI" autocomplete="off" class="w-full p-2 border border-gray-300 rounded-lg" required />
            <p v-if="dniError" class="text-red-500">El DNI debe contener exactamente 10 números.</p>
          </div>
          <div class="mb-4">
            <label for="telefono" class="block text-sm font-medium text-gray-700">Teléfono</label>
            <input type="tel" id="telefono" v-model="ComTelefono" @input="validatePhone" autocomplete="off" class="w-full p-2 border border-gray-300 rounded-lg" required />
            <p v-if="phoneError" class="text-red-500">El teléfono debe contener solo números.</p>
          </div>
...
export default {
  ...
  methods: {
    validateDNI() {
      // Validar que el DNI contenga solo números y tenga exactamente 10 dígitos
      const dniPattern = /^\d{10}$/;
      this.dniError = !dniPattern.test(this.ComDni);
    },
    validatePhone() {
      // Validar que el teléfono contenga solo números
      const phonePattern = /^\d+$/;
      this.phoneError = !phonePattern.test(this.ComTelefono);
    },
